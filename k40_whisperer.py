...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.PreviewCanvas = Canvas(
            lbframe, width=self.w - (220 + 20), height=self.h - 200, background="grey75"
        )
        ...
        self.Reng_Veng_Button = Button(
            self.master, text="Raster and\nVector Engrave", command=self.Raster_Vector_Eng
        )
        self.Veng_Vcut_Button = Button(
            self.master, text="Vector Engrave\nand Cut", command=self.Vector_Eng_Cut
        )
        self.Reng_Veng_Vcut_Button = Button(
            self.master,
            text="Raster Engrave\nVector Engrave\nand\nVector Cut",
            command=self.Raster_Vector_Cut,
        )

        self.Label_Position_Control = Label(self.master, text="Position Controls:", anchor=W)

        self.Initialize_Button = Button(
            self.master, text="Initialize Laser Cutter", command=self.Initialize_Laser
        )

        self.Open_Button = Button(
            self.master, text="Open\nDesign File", command=self.menu_File_Open_Design
        )
        self.Reload_Button = Button(
            self.master, text="Reload\nDesign File", command=self.menu_Reload_Design
        )
        ...
        self.halftone.trace_variable(
            "w", self.View_Refresh_and_Reset_RasterPath
        ...
        self.Entry_Reng_passes.configure(
            textvariable=self.Reng_passes, justify="center", fg="black"
        )
        ...
        self.Entry_Gcde_passes.configure(
            textvariable=self.Gcde_passes, justify="center", fg="black"
        )
        ...
        self.calc_button = Button(
            self.master, text="Calculate Raster Time", command=self.menu_Calc_Raster_Time
        )
        ...
        top_File.add(
            "command", label="Read Settings File", command=self.menu_File_Open_Settings_File
        )

        top_File.add_separator()
        top_File.add(
            "command", label="Open Design (SVG/DXF/G-Code)", command=self.menu_File_Open_Design
        )
        ...
        SaveEGVmenu.add(
            "command",
            label="Raster and Vector Engrave",
            command=self.menu_File_Raster_Vector_Engrave,
        )
        SaveEGVmenu.add(
            "command", label="Vector Engrave and Cut", command=self.menu_File_Vector_Engrave_Cut
        )
        SaveEGVmenu.add(
            "command",
            label="Raster, Vector Engrave and Vector Cut",
            command=self.menu_File_Raster_Vector_Cut,
        )
        ...
        top_View.add_checkbutton(
            label="Show Raster Image", variable=self.include_Reng, command=self.menu_View_Refresh
        )
        if DEBUG:
            top_View.add_checkbutton(
                label="Show Raster Paths",
                variable=self.include_Rpth,
                command=self.menu_View_Refresh,
            )

        top_View.add_checkbutton(
            label="Show Vector Engrave", variable=self.include_Veng, command=self.menu_View_Refresh
        )
        top_View.add_checkbutton(
            label="Show Vector Cut", variable=self.include_Vcut, command=self.menu_View_Refresh
        )
        top_View.add_checkbutton(
            label="Show G-Code Paths", variable=self.include_Gcde, command=self.menu_View_Refresh
        )
        top_View.add_separator()
        top_View.add_checkbutton(
            label="Show Time Estimates", variable=self.include_Time, command=self.menu_View_Refresh
        )
        top_View.add_checkbutton(
            label="Zoom to Design Size", variable=self.zoom2image, command=self.menu_View_Refresh
        )
        ...
        top_Tools.add(
            "command", label="Trace Design Boundary <Ctrl-t>", command=self.TRACE_Settings_Window
        )
        ...
        top_Settings.add(
            "command", label="Raster Settings <F3>", command=self.RASTER_Settings_Window
        )
        top_Settings.add(
            "command", label="Rotary Settings <F4>", command=self.ROTARY_Settings_Window
        )
        top_Settings.add_separator()
        top_Settings.add_checkbutton(
            label="Advanced Settings <F6>", variable=self.advanced, command=self.menu_View_Refresh
        )
    ...
    def refreshTime(self):
        ...
        if self.VengData.len != None:
            Veng_time = (
                self.VengData.len / Vector_eng_feed + self.VengData.move / rapid_feed
            ) * Vector_eng_passes
        if self.VcutData.len != None:
            Vcut_time = (
                self.VcutData.len / Vector_cut_feed + self.VcutData.move / rapid_feed
            ) * Vector_cut_passes
        ...
        if self.GcodeData.ecoords == []:
            self.PreviewCanvas.create_text(
                HUD_X, HUD_Y, fill="red", text=self.Vcut_time.get(), anchor="se", tags="HUD"
            )
            self.PreviewCanvas.create_text(
                HUD_X,
                HUD_Y - HUD_vspace,
                fill="blue",
                text=self.Veng_time.get(),
                anchor="se",
                tags="HUD",
            )
            ...
            if Reng_time == None:
                ...
                self.PreviewCanvas.create_text(
                    ...
                    HUD_Y - HUD_vspace * 2,
                    fill="black",
                    text=self.Reng_time.get(),
                    anchor="se",
                    tags="HUD",
                )
        else:
            self.PreviewCanvas.create_text(
                HUD_X, HUD_Y, fill="black", text=self.Gcde_time.get(), anchor="se", tags="HUD"
            )
        ##########################################
    ...
    def Entry_Laser_Rapid_Feed_Check(self):
        try:
            ...
            if value != 0 and value < low_limit:
                self.statusMessage.set(
                    " Rapid feed should be greater than or equal to %f (or 0 for default speed) "
                    % (low_limit)
                )
                return 2  # Value is invalid number
        except:
    ...
    def Inkscape_Path_Click(self, event):
        ...
        win_id = self.grab_current()
        newfontdir = askopenfilename(
            filetypes=[("Executable Files", ("inkscape.exe", "*inkscape*")), ("All Files", "*")],
    ...
    def menu_File_Open_Settings_File(self, event=None):
        ...
        fileselect = askopenfilename(
            filetypes=[("Settings Files", "*.txt"), ("All Files", "*")], initialdir=init_dir
        )
    ...
    def menu_File_Open_EGV(self):
        ...
        if not os.path.isdir(init_dir):
            init_dir = self.HOME_DIR
        fileselect = askopenfilename(
            filetypes=[("Engraver Files", ("*.egv", "*.EGV")), ("All Files", "*")],
            initialdir=init_dir,
        )
        if fileselect != "" and fileselect != ():
            self.resetPath()
    ...
    def Open_SVG(self, filemname):
        ...
        if (
            self.Design_bounds[0] > self.VengData.bounds[0] + margin
            or self.Design_bounds[0] > self.VcutData.bounds[0] + margin
            or self.Design_bounds[1] < self.VengData.bounds[1] - margin
            or self.Design_bounds[1] < self.VcutData.bounds[1] - margin
            or self.Design_bounds[2] > self.VengData.bounds[2] + margin
            or self.Design_bounds[2] > self.VcutData.bounds[2] + margin
            or self.Design_bounds[3] < self.VengData.bounds[3] - margin
            or self.Design_bounds[3] < self.VcutData.bounds[3] - margin
        ):
            line1 = "Warning:\n"
    ...
    def make_raster_coords(self):
        ...
        try:
            ...
            if self.RengData.image != None and self.RengData.ecoords == []:
                ...
                stepIndex = 0  # index of the line being processed
                SCAN_OFFSET = float(
                ...
                for i_step in range(0, im_height_mils, Raster_step):
                    ...
                    for i in rng:
                        ...
                        if seg[1] == "D":
                            ...
                            ######################
                            if stepIndex % 2 == 0:
                                offset = SCAN_OFFSET  # offset the scanline by the scan offset every other line
                            else:
                                offset = 0
                            ecoords.append(
                                [x + offset, y, loop]
                            )  # add the scanline to the list of scanlines (with possible offset)
                            ecoords.append([x + delta + offset, y, loop])
                            #######################
    ...
    def gcode_error_message(self, message):
        ...
        Error_Text = Text(
            Error_Frame, width="80", height="20", yscrollcommand=scrollbar.set, bg="white"
        )
        for line in message:
    ...
    def Move_LR(self, dummy=None):
        ...
        (Xsize, Ysize) = self.LASER_Size()
        if Xnew <= Xsize + 0.001 and Ynew >= -Ysize - 0.001:
    ...
    def Raster_Vector_Cut(self, output_filename=None):
        ...
        try:
            ...
            if (
                ...
                or self.VengData.ecoords != []
                or self.VcutData.ecoords != []
            ):
    ...
    def Finish_Job(self, event=None):
        ...
        if self.post_exec.get():
            ...
            startupinfo = None
            proc = Popen(
                cmd, shell=False, stdout=PIPE, stderr=PIPE, stdin=PIPE, startupinfo=startupinfo
            )
        ...
        try:
            ...
            if (operation_type.find("Vector_Cut") > -1) and (self.VcutData.ecoords != []):
                ...
                if not self.VcutData.sorted and self.inside_first.get():
                    self.VcutData.set_ecoords(
                        self.optimize_paths(self.VcutData.ecoords), data_sorted=True
                    )
            ...
            if (operation_type.find("Vector_Eng") > -1) and (self.VengData.ecoords != []):
                ...
                if not self.VengData.sorted and self.inside_first.get():
                    self.VengData.set_ecoords(
                        self.optimize_paths(self.VengData.ecoords, inside_check=False),
                        data_sorted=True,
                    )
                self.statusMessage.set("Generating EGV data...")
                self.master.update()
            ...
            if (operation_type.find("Gcode_Cut") > -1) and (self.GcodeData.ecoords != []):
                ...
                Gcode_coords, startx, starty = self.scale_vector_coords(
                    Gcode_coords, startx, starty
                )
    ...
    def menu_View_Refresh(self):
        ...
        self.statusMessage.set(
            " Current Position: X=%.3f Y=%.3f    ( W X H )=( %.3f%s X %.3f%s ) "
            % (X_display, Y_display, W_display, U_display, H_display, U_display)
        )
        ...
        if abs(self.w - w) > 10 or abs(self.h - h) > 10 or update == 1:
            ...
            if True:
                ...
                if h >= 560:
                    ...
                    Yloc = Yloc + 6
                    self.Label_Position_Control.place(
                ...
                if self.GcodeData.ecoords == []:
                    ...
                    if self.comb_vector.get() or self.comb_engrave.get():
                        ...
                        if self.comb_engrave.get():
                            if self.comb_vector.get():
                                self.Reng_Veng_Vcut_Button.place(
                                    x=12, y=Y_Reng, width=100, height=23 * 3 + 14
                                )
                            else:
                                self.Reng_Veng_Button.place(
                                    x=12, y=Y_Reng, width=100, height=23 * 2 + 7
                                )
                        elif self.comb_vector.get():
                            self.Veng_Vcut_Button.place(
                                x=12, y=Y_Veng, width=100, height=23 * 2 + 7
                            )
                ...
                if self.advanced.get():
                    ...
                    adv_Yloc = 25 - 10  # 15
                    self.Label_Advanced_column.place(
                        x=Xadvanced, y=adv_Yloc, width=wadv_use, height=21
                    )
                    ...
                    if h >= 560:
                        ...
                        self.Label_Halftone_adv.place(
                            x=Xadvanced, y=adv_Yloc, width=w_label_adv, height=21
                        )
                        self.Checkbutton_Halftone_adv.place(
                            x=Xadvanced + w_label_adv + 2, y=adv_Yloc, width=25, height=23
                        )

                        adv_Yloc = adv_Yloc + 25
                        self.Label_Negate_adv.place(
                            x=Xadvanced, y=adv_Yloc, width=w_label_adv, height=21
                        )
                        self.Checkbutton_Negate_adv.place(
                            x=Xadvanced + w_label_adv + 2, y=adv_Yloc, width=25, height=23
                        )
                        ...
                        self.Label_Mirror_adv.place(
                            x=Xadvanced, y=adv_Yloc, width=w_label_adv, height=21
                        )
                        self.Checkbutton_Mirror_adv.place(
                            x=Xadvanced + w_label_adv + 2, y=adv_Yloc, width=25, height=23
                        )

                        adv_Yloc = adv_Yloc + 25
                        self.Label_Rotate_adv.place(
                            x=Xadvanced, y=adv_Yloc, width=w_label_adv, height=21
                        )
                        self.Checkbutton_Rotate_adv.place(
                            x=Xadvanced + w_label_adv + 2, y=adv_Yloc, width=25, height=23
                        )

                        adv_Yloc = adv_Yloc + 25
                        self.Label_inputCSYS_adv.place(
                            x=Xadvanced, y=adv_Yloc, width=w_label_adv, height=21
                        )
                        self.Checkbutton_inputCSYS_adv.place(
                            x=Xadvanced + w_label_adv + 2, y=adv_Yloc, width=25, height=23
                        )
                        ...
                        adv_Yloc = adv_Yloc + 25 - 20
                        self.Label_Inside_First_adv.place(
                            x=Xadvanced, y=adv_Yloc, width=w_label_adv, height=21
                        )
                        self.Checkbutton_Inside_First_adv.place(
                            x=Xadvanced + w_label_adv + 2, y=adv_Yloc, width=25, height=23
                        )

                        adv_Yloc = adv_Yloc + 25
                        self.Label_Rotary_Enable_adv.place(
                            x=Xadvanced, y=adv_Yloc, width=w_label_adv, height=21
                        )
                    ...
                    if self.GcodeData.ecoords == []:
                        ...
                        self.Label_Vcut_passes.place(
                            x=Xadvanced, y=Y_Vcut, width=w_label_adv, height=21
                        )
                        self.Entry_Vcut_passes.place(
                            x=Xadvanced + w_label_adv + 2, y=Y_Vcut, width=w_entry, height=23
                        )

                        # adv_Yloc=adv_Yloc-30
                        self.Label_Veng_passes.place(
                            x=Xadvanced, y=Y_Veng, width=w_label_adv, height=21
                        )
                        self.Entry_Veng_passes.place(
                            x=Xadvanced + w_label_adv + 2, y=Y_Veng, width=w_entry, height=23
                        )

                        # adv_Yloc=adv_Yloc-30
                        self.Label_Reng_passes.place(
                            x=Xadvanced, y=Y_Reng, width=w_label_adv, height=21
                        )
                        self.Entry_Reng_passes.place(
                            x=Xadvanced + w_label_adv + 2, y=Y_Reng, width=w_entry, height=23
                        )
                        ...
                        self.separator_comb.place(
                            x=Xadvanced - 1, y=adv_Yloc, width=wadv_use, height=2
                        )

                        adv_Yloc = adv_Yloc - 25
                        self.Label_Comb_Vector_adv.place(
                            x=Xadvanced, y=adv_Yloc, width=w_label_adv, height=21
                        )
                        self.Checkbutton_Comb_Vector_adv.place(
                            x=Xadvanced + w_label_adv + 2, y=adv_Yloc, width=25, height=23
                        )

                        adv_Yloc = adv_Yloc - 25
                        self.Label_Comb_Engrave_adv.place(
                            x=Xadvanced, y=adv_Yloc, width=w_label_adv, height=21
                        )
                        self.Checkbutton_Comb_Engrave_adv.place(
                            x=Xadvanced + w_label_adv + 2, y=adv_Yloc, width=25, height=23
                        )
                        ####
                        ...
                        adv_Yloc = adv_Yloc - 40
                        self.Label_Gcde_passes.place(
                            x=Xadvanced, y=adv_Yloc, width=w_label_adv, height=21
                        )
                        self.Entry_Gcde_passes.place(
                            x=Xadvanced + w_label_adv + 2, y=adv_Yloc, width=w_entry, height=23
                        )
    ...
    def Plot_Data(self):
        ...
        if min((xmax - xmin), (ymax - ymin)) > 0 and self.zoom2image.get():
            ...
            if self.HomeUR.get():
                x_rgt = (
                    (xmax - minx) / self.PlotScale
                    - self.laserX / self.PlotScale
                    + (cszw - (xmax - xmin) / self.PlotScale) / 2
                )
                x_lft = (
                    (xmax - maxx) / self.PlotScale
                    - self.laserX / self.PlotScale
                    + (cszw - (xmax - xmin) / self.PlotScale) / 2
                )
            else:
            ...
            y_bot = (
                -miny / self.PlotScale
                + self.laserY / self.PlotScale
                + (cszh - (ymax - ymin) / self.PlotScale) / 2
            )
            y_top = (
                -maxy / self.PlotScale
                + self.laserY / self.PlotScale
                    ...
                    if new_SCALE != self.SCALE:
                        ...
                        try:
                            self.UI_image = ImageTk.PhotoImage(
                                plot_im.resize((nw, nh), Image.LANCZOS)
                            )
                        except:
                            debug_message("Imaging_Free Used.")
                            self.UI_image = self.Imaging_Free(
                                plot_im.resize((nw, nh), Image.LANCZOS)
                            )
                except:
                    self.SCALE = 1
        ...
        if self.trace_window.winfo_exists():  # or DEBUG:
            ...
            for i in range(len(trace_coords)):
                trace_coords[i] = [
                    trace_coords[i][0] * Xscale,
                    trace_coords[i][1] * Yscale,
                    trace_coords[i][2],
                ]
    ...
    def Plot_Raster(self, XX, YY, Xleft, Ytop, PlotScale, im):
        ...
        self.segID.append(
            self.PreviewCanvas.create_image(
                xplt, yplt, anchor=NW, image=self.UI_image, tags="LaserTag"
            )
        )
    ...
    def Plot_Line(
        ...
        self.segID.append(
            self.PreviewCanvas.create_line(
                xplt1, yplt1, xplt2, yplt2, fill=col, capstyle="round", width=thick, tags=tag_value
            )
        )
    ...
    def GEN_Settings_Window(self):
        ...
        self.Checkbutton_post_exec = Checkbutton(
            gen_settings, text="Run Batch File:", anchor=W, command=self.Set_Input_States_BATCH
        )
        ...
        self.Checkbutton_Reduce_Memory = Checkbutton(
            gen_settings, text="(needed for large designs or low memory computers)", anchor=W
        )
        ...
        self.Checkbutton_Wait = Checkbutton(
            gen_settings, text="(after all data has been sent over USB)", anchor=W
        )
        ...
        self.Inkscape_Path.place(
            x=xd_entry_L + font_entry_width + 10, y=D_Yloc, width=110, height=23
        )
    ...
    def RASTER_Settings_Window(self):
        ...
        self.Checkbutton_Halftone = Checkbutton(
            raster_settings, text=" ", anchor=W, command=self.Set_Input_States_RASTER
        )
        ...
        if self.reduced_mem.get():
            ...
            if self.ht_size == "143":
                self.ht_size = "167"
            self.Halftone_DPI_OptionMenu = OptionMenu(
                raster_settings, self.ht_size, "500", "250", "167", "125"
            )
        else:
            self.Halftone_DPI_OptionMenu = OptionMenu(
                raster_settings,
        ...
        self.Label_bezier_M1 = Label(
            raster_settings, text="Slope, Black (%.1f)" % (self.bezier_M1_default), anchor=CENTER
        )
        self.bezier_M1_Slider = Scale(
            raster_settings,
            from_=1,
            to=50,
            resolution=0.1,
            orient=HORIZONTAL,
            variable=self.bezier_M1,
        )
        ...
        self.Label_bezier_M2 = Label(
            raster_settings, text="Slope, White (%.2f)" % (self.bezier_M2_default), anchor=CENTER
        )
        self.bezier_M2_Slider = Scale(
            raster_settings,
            from_=0.0,
            to=1,
            orient=HORIZONTAL,
            resolution=0.01,
            variable=self.bezier_M2,
        )
        ...
        D_Yloc = D_Yloc + D_dY - 8
        self.Label_bezier_weight = Label(
            raster_settings, text="Transition (%.1f)" % (self.bezier_M1_default), anchor=CENTER
        )
        self.bezier_weight_Slider = Scale(
            raster_settings,
            from_=0,
            to=10,
            resolution=0.1,
            orient=HORIZONTAL,
            variable=self.bezier_weight,
        )
        self.bezier_weight_Slider.place(
            x=xd_entry_L, y=D_Yloc, width=(Wset - xd_entry_L - 25 - 280)
        )
        D_Yloc = D_Yloc + 21
        ...
        self.BezierCanvas.create_line(
            5, 260 - 0, 260, 260 - 255, fill="grey75", capstyle="round", width=2, tags="perm"
        )
    ...
    def ROTARY_Settings_Window(self):
        ...
        self.Checkbutton_Rotary_Enable = Checkbutton(
            rotary_settings, text="", anchor=W, command=self.Set_Input_States_Rotary
        )
    ...
    def TRACE_Settings_Window(self, dummy=None):
        ...
        self.Trace_Button = Button(
            trace_window, text="Trace Boundary With Laser Head", command=Close_and_Send_Click
        )
...
class pxpiDialog(tkSimpleDialog.Dialog):
    def __init__(
        self, parent, units="mm", SVG_Size=None, SVG_ViewBox=None, SVG_inkscape_version=None
    ):
        self.result = None
